class PerlinNoise:
    permutation: list = [151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103,
    ...
    def __init__(self, repeat: int = -1):
        self.p: list = [x % 256 for x in range(0, 512)]
        self.repeat = repeat

    def octave_perlin(self, x: float, y: float, z: float, octaves: int, persistence: float) -> float:
        total: float = 0
        frequency: float = 1
        amplitude: float = 1
        max_value: float = 0
    ...
    def perlin(self, x: float, y: float, z: float) -> float:
        # If repeat on, change the coordinates to their 'local' repetitions.
        if self.repeat > 0:
            x = x % self.repeat
            y = y % self.repeat
            z = z % self.repeat
        ...
        # Value used with interpolation
        u: float = self.fade(xf)
        v: float = self.fade(yf)
        w: float = self.fade(zf)
    ...
    def inc(self, num: int) -> int:
        """
        Increment the number and make sure that the noise still repeats
        :param num:
        :return:
        """
        num += num + 1
